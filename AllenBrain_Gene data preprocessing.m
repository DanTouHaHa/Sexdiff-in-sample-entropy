...
cd('E:\Fudan_Luoqiang_MDDProject\848MDD_794NC\数据分析\数据整理统计分析\富集分析基因表达\AllenBrain/');
...
work_dir = 'E:/Fudan_Luoqiang_MDDProject/848MDD_794NC/数据分析/数据整理统计分析/富集分析基因表达/AllenBrain';
Yraw = readtable('E:\Fudan_Luoqiang_MDDProject\848MDD_794NC\数据分析\数据整理统计分析\富集分析基因表达\AllenBrain\Dosen160_NC_Sex_Node.csv'); 
...
feature_total = 6;
Yraw_new(:,8:13) = zscore(table2array(Yraw(:,8:13)));
% corr(table2array(Yraw(:,8:13)))

XYZ = [Yraw.x_mm_,Yraw.y_mm_,Yraw.z_mm_];
XYZ_lables = Yraw.Labels';
%%
...
for feature = 1:feature_total
    Y = Yraw_new(in_rois,7+feature); %column 5~8 features:{'FHC'}    {'FMDD'}    {'MHC'}    {'MMDD'}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%picture start
    ...
    subplot(2,feature_total,feature);
%     figure
...
%     l.Color = 'k';
    ...
    subplot(2,feature_total,feature_total+feature);
%     fig = figure;
...
clc
bootnum=5000;
Y = Yraw_new(in_rois,13);%column 8~13 features:
...
cd (work_dir)
writetable(sortrows(PLS_out_genes,-3),'PLS_out_genes_cort_PLS_variance1.csv');
writetable(sortrows(PLS2_out_genes,-3),'PLS_out_genes_cort_PLS_variance2.csv');
writetable(sortrows(PLS3_out_genes,-3),'PLS_out_genes_cort_PLS_variance_max.csv');
cd '..'
